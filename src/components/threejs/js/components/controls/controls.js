...
export class Controls {
  ...
  constructor (camera, container, scene) {
    ...
    if (threeConfig.isMobile) {
      ...
      // #region
      if (threeConfig.controls.mobile.orbitControlsEnabled) this._orbitControls = new OrbitControls(this._camera, this._container)
      if (threeConfig.controls.mobile.orbitControlsEnabled) {
        this._orbitControls.minAzimuthAngle = threeConfig.controls.mobile.minAzimuthAngle
        this._orbitControls.maxAzimuthAngle = threeConfig.controls.mobile.maxAzimuthAngle
        this._orbitControls.enableZoom = threeConfig.controls.mobile.enableZoom
        this._orbitControls.minDistance = threeConfig.controls.mobile.minDistance
        this._orbitControls.maxDistance = threeConfig.controls.mobile.maxDistance
        if (!threeConfig.controls.mobile.infinitePolar) {
          this._orbitControls.maxPolarAngle = threeConfig.controls.mobile.maxPolarAngle
          this._orbitControls.minPolarAngle = threeConfig.controls.mobile.minPolarAngle
          this._orbitControls.enableDamping = threeConfig.controls.mobile.enableDamping
        }
        if (threeConfig.controls.mobile.enableDamping) {
          this._orbitControls.dampingFactor = threeConfig.controls.mobile.dampingFactor
          this._orbitControls.enableZoom = threeConfig.controls.mobile.zoomEnabled
        }
      }
      // #endregion
      ...
      // #region
      if (threeConfig.controls.mobile.dragEnabled) this._dragControls = new DragControls([], this._camera, this._container)
      // #endregion
    } else {
      // orbit controls (non mobile)
      // #region
      if (threeConfig.controls.orbitControlsEnabled) this._orbitControls = new OrbitControls(this._camera, this._container)
