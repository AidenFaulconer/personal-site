...
class Main extends View {
  constructor (canvas) { // canvas div to pass into view
    ...
    // handle mobile
    // #region
    if (threeConfig.isMobile) { // delete unused objects
    ...
    if (threeConfig.background.enabled) {
      const texture = new CubeTextureLoader().load([backgroundEmpty, backgroundEmpty, backgroundEmpty, backgroundEmpty, backgroundEmpty, backgroundEmpty])
      texture.minFilter = LinearFilter
      this._scene.background = texture
    } else {
      this._scene.background = new Color(threeConfig.background.color)
    }
    threeConfig.fog.isFog ? this._scene.fog = new Fog(threeConfig.fog.color, threeConfig.fog.density, threeConfig.fog.far) : console.log('not rendering fog')
